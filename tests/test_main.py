...
def test_write_chunked_text_files(mock_logger, tmp_path):
    ...
    content = ""
    paragraph_template = "This is a test paragraph for chunking. It is designed to be long enough to contribute significantly to the chunk size. We will repeat this paragraph multiple times to create content that needs to be split into several chunks. " * 100 # Roughly 2000 characters
    ...
    long_paragraph = "This is a very long paragraph that should be split into sentences. " * 500 # Roughly 100000 characters, ~25000 tokens
    ...
    for i, file_path in enumerate(chunk_files):
        ...
        if i < len(chunk_files) - 1:
             ...
             assert estimated_tokens <= chunk_size * 1.5 # Last chunk can be larger if a sentence/paragraph pushed it over

        mock_logger.info.assert_any_call(f"Successfully wrote chunk {i+1}/{len(chunk_files)} for {package_name} to {file_path}")
