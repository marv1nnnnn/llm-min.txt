...
from src.llm_min.search import (
...
def test_search_for_documentation_urls_success(mock_ddgs):
    ...
    mock_instance.text.assert_called_once_with(f"{package_name} document website", max_results=10)
...
def test_search_for_documentation_urls_custom_num_results(mock_ddgs):
    ...
    mock_ddgs.assert_called_once()
    mock_instance.text.assert_called_once_with(f"{package_name} document website", max_results=num_results)
    assert len(results) == num_results


# Mock generate_text_response for select_best_url_with_llm tests
@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_success(mock_generate_text_response):
    ...
    selected_url = select_best_url_with_llm(package_name, search_results, api_key=api_key)

    mock_generate_text_response.assert_called_once()
    assert (
        "Analyze the following search results for the Python package 'test_package'."
        in mock_generate_text_response.call_args[0][0]
    )
    assert mock_generate_text_response.call_args[1]["api_key"] == api_key
    assert selected_url == "http://example.com/official/docs"


@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_returns_none(mock_generate_text_response):
    ...
    selected_url = select_best_url_with_llm(package_name, search_results)
...
@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_returns_empty_string(mock_generate_text_response):
...
@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_invalid_url_format(mock_generate_text_response):
...
@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_empty_search_results(mock_generate_text_response):
    ...
    mock_generate_text_response.assert_not_called()
    assert selected_url is None


@patch("src.llm_min.search.generate_text_response")
def test_select_best_url_with_llm_exception(mock_generate_text_response):
...
@patch("src.llm_min.search.select_best_url_with_llm")
@patch("src.llm_min.search.search_for_documentation_urls")
def test_find_documentation_url_success(mock_search, mock_select_url):
    ...
    package_name = "test_package"
    api_key = "fake_api_key"
    found_url = find_documentation_url(package_name, api_key=api_key)

    mock_search.assert_called_once_with(package_name)
    mock_select_url.assert_called_once_with(package_name, mock_search.return_value, api_key=api_key)
    assert found_url == "http://example.com/official/docs"


@patch("src.llm_min.search.select_best_url_with_llm")
@patch("src.llm_min.search.search_for_documentation_urls")
def test_find_documentation_url_no_search_results(mock_search, mock_select_url):
    """Tests find_documentation_url when search returns no results."""
    mock_search.return_value = []

    package_name = "test_package"
    found_url = find_documentation_url(package_name)

    mock_search.assert_called_once_with(package_name)
    mock_select_url.assert_not_called()
    assert found_url is None


@patch("src.llm_min.search.select_best_url_with_llm")
@patch("src.llm_min.search.search_for_documentation_urls")
def test_find_documentation_url_llm_returns_none(mock_search, mock_select_url):
    ...
    mock_search.assert_called_once_with(package_name)
    mock_select_url.assert_called_once_with(package_name, mock_search.return_value, api_key=None)
    assert found_url is None


@patch("src.llm_min.search.select_best_url_with_llm")
@patch("src.llm_min.search.search_for_documentation_urls")
def test_find_documentation_url_url_cleaning(mock_search, mock_select_url):
    ...
    for raw_url, cleaned_url in test_cases:
        ...
        mock_search.assert_called_once_with(package_name)  # Ensure search is called
        mock_select_url.assert_called_once_with(
            package_name, mock_search.return_value, api_key=None
        )  # Ensure select is called
        mock_search.reset_mock()  # Reset mocks for next iteration
